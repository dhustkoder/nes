...
void foo(void)
{
	...
	PPU_VRAM_ADDR = (0x20 + (y >> 3)); /* here, we take only the bit 7 of y and add to 0x20 */
	/* but y is only 1: 0x01 , so 0x20 stays 0x20  */
	/* 0x2000 in ppu.vram.addr + 1 now ...*/

	PPU_VRAM_ADDR = ((y << 5) + x); /* now we're at other 8 bits of ppu.vram.addr
	then y must be multiplied by 32 and plus x which is 33 too */
	/* 0x2021 now at ppu.vram.addr + 1 */

	/* ok now we can write one byte at time, and it will be displayed */
	/* and increase the offset 1,1 ... 2, 1 .... 3, 1*/
